...
<html>
<head>
<title><%=((InsegnanteBean)session.getAttribute("utente")).getNome() %> <%=((InsegnanteBean)session.getAttribute("utente")).getCognome() %></title>le>
<meta charset="utf-8">
	...
	<div class="container">
		<%
			...
			for (int i = 0; i < lezioni.size(); i++) {
				...
				if (y.getDataLezione().after(today)) {
					lezioniPro.add(y);
				}
			}
			...
			<%if (lezioniPro.size()!=0){ 
				...
				<div>
					...
					<h4>
							...
							<a href="AnnullaLezione?cod=<%=y.getCodice() %>">
						<button type="button" class="btn btn-primary" >Annulla lezione</button>
						</a>
